...
<script id="appsListTemplate" type="text/x-jsrender">
    <li id="{{:id}}" >
        ...
        <div class="txt">
            ...
            <span class="intro">{{:memo}}</span>
        </div>
        <div class="mod-btn mod-btn-official">
            <a class="btn-install appdown" href="../appres/app_files/{{:apkurl}}">安装</a>
        </div>
    </li>
</script>
<script type="text/javascript">
    ...
    function loadMoreApp() {
        ...
        $.post("servlet/AppServlet?method=GetClientAppList", { id: cid,key:trim($("#keyword").val())}, function (data) {
    ...
    function searchAPP() {
        if (searchkeywords != "") {
            $.post("servlet/AppServlet?method=GetClientAppList", { id:0,key:trim($("#keyword").val())}, function (data) {
                var template = $.templates("#appsListTemplate");
                var htmlOutput = template.render(data);
