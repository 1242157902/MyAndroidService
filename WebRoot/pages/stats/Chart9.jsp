...
<head>
    ...
    function statis_mbinfo(){  
       ...
       if(staxunit=='month'&&getDays(staregstart,staregend)>365) {
		...
		$.post("${pageContext.request.contextPath }/servlet/StServlet?method=GetChart7Data",{
		start:staregstart,
		end:staregend,
		xunit:staxunit,
		tjtj:statjtj,
		...
		<div data-options="region:'north',height:150,title:'查询条件'">
			<form id="mbinfo_form" method="post">
				...
				<div style="padding-top:5px;padding-bottom:5px;">
				   ...
				   统计项目<select id="mbinfo_xunit" name="mbinfo_xunit"  style="width:120px"></select>
				   统计目标<select id="mbinfo_tjtj" name="mbinfo_tjtj"  style="width:120px"></select>					           
...
<script>
	$(function(){	
		var xunit = [
			{value:'sex',text:'性别'},
			{value:'age',text:'年龄'},
			{value:'unit',text:'所属单位'},
			{value:'seller',text:'销售单位'},
			{value:'brand',text:'手机类型'},
			{value:'provider',text:'运营商'},
			{value:'area',text:'归属地'}
		];		
		var atjtj = [
 ...
 function showPieChart(chartdata){
	var chart9title='滑屏次数';
	var chart9unit='次';
	if($('#mbinfo_tjtj').combobox('getValue')=='score') {chart9title='滑屏积分';chart9unit='分';}
