...
<script type="text/javascript"	src="js/jquery-easyui-1.4.1/jquery.easyui.min.js"></script>	
	...
	$("#scorecolor").val(window.parent.document.getElementById("cscolor").innerHTML);	
	document.getElementById("link").value=window.parent.document.getElementById("clink").innerHTML.substring(7);
	document.getElementById("imgA").src="images/pics/"+window.parent.document.getElementById("cnamex").innerHTML;
	document.getElementById("imgB").src="images/pics/"+window.parent.document.getElementById("cnamex").innerHTML;
	
	var ciconviewtempstr="";
    var ciconviewdata= window.parent.document.getElementById("ciconstr").innerHTML;
    var ciconview1= ciconviewdata.split(',');
    if(ciconviewdata==null||ciconviewdata=="undefined"|| ciconviewdata=="") ciconview1=[]; 
    for(i=0;i<ciconview1.length;i++)   {
     var  ciconview2= ciconview1[i].split('*');
     ciconviewtempstr=ciconviewtempstr+"<li style='float:left;height:60px;'><img src='images/icons/"+ciconview2[0]+"'  title='"+ciconview2[2]+"' alt='加载错误' width=60 height=60 /></li>";
    }
    $("#iconViewList").html(ciconviewtempstr);
	});
   ...
   function initimgA()
   {
     var AAoffwidth= document.getElementById('imgA').offsetWidth;
     var AAoffheight= document.getElementById('imgA').offsetHeight;
     var rect = clacImgZoomParam(MAXWIDTH, MAXHEIGHT,AAoffwidth,AAoffheight); 
     $('#imgApane').css({
        width: rect.width,
        height:rect.height,
        marginLeft: '0px',
  ...
  <table  border="1" cellpadding="1"  bordercolor="gray"> 
       <tr>                             
           <td colspan=3 style="text-align:center;" >修改内容信息  <input id="cid" type="hidden" name="cid" />  </td>   
           <td rowspan=8 style="width:600px;height:400px;"  valign="top">
             <div id="imgApane" ><div id=imgAdiv><img id="imgA"  onload="initimgA()" /> </div>   </div>  
           </td>                
       </tr>      
       <tr>                             
      ...
      <tr>                             
       ...
       <td style="text-align:left;padding-left:2px;font-size:12px;"  colspan=2>
        <input id="aclassify" name="classify" class="easyui-combotree" data-options="url:'servlet/JsonServlet?method=GetContentClassList',required:true,missingMessage:'不能为空!',onLoadSuccess:function(){$('#aclassify').combotree('setValue',window.parent.document.getElementById('classify').innerHTML);}"  style="width:270px;height:30px;cursor:pointer">
       </td>
       </tr> 
       <tr>                             
 ...
 <input id=SaveNewIconcBtn type="hidden"  onClick="SaveNewIconc()" />  
    ...
    // alert("x="+$('#imgx').val()+",y="+$('#imgy').val()+",w="+$('#imgw').val()+",h="+$('#imgh').val());
	  ...
	  jQuery("#contentform").attr('action','servlet/CtServlet?method=UpdateContent&filenamex='+window.parent.document.getElementById("cnamex").innerHTML);
  ...
  function hehe() {
    ...
    $('#imgAdiv').Jcrop({
      ...
      var y=boundy;
      if(x*1.7<y) y=x*1.7;
      else x=y/1.7;//alert(Math.round(170*boundy/y));      
